...
def extract_body_ids_and_launch(c, args, seg_instance, body_csv, msgs_df):
    ...
    # FIXME: Instead of hard-coding fields like -W, -P, -q, read the template dask-config.yaml
    cmd = (f"source $({args.conda_path} info --base)/bin/activate {args.conda_env} "
           f"&& cd {args.cwd} "
           f"&& bsub -P flyem -q local -W 01:00 -n {args.driver_slots} -o /dev/null launchflow -n {args.worker_slots} {args.template_dir}")
