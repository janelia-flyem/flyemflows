...
def roistats_table(input_path=ROISTATS_CSV, voxel_col=VOXEL_COL, output_path=OUTPUT_PATH):
    ...
    This function appends fractional columns to the absolute numbers, adds synapse counts
    (from neuprint) filters for the top-N ROIs for each body, and pivots (reshapes) the
    data so that the top-5 ROIs (and all stats) can be viewed on a single row of a
    spreadsheet.
